...
namespace Wxpay
{
    public class WxPayHelper
    {
        ...
        public string GetCftPackage()
        {
            ...
            string unSignParaString = Wxpay.CommonUtil.FormatBizQueryParaMap(parameters,
                    false);
            string paraString = Wxpay.CommonUtil.FormatBizQueryParaMap(parameters, true);
            return paraString + "&sign="
                    + Wxpay.MD5SignUtil.Sign(unSignParaString, PartnerKey);
        ...
        public string GetBizSign(Dictionary<string, string> bizObj)
        {
            ...
            string bizString = Wxpay.CommonUtil.FormatBizQueryParaMap(bizParameters, false);

            return Wxpay.SHA1Util.Sha1(bizString);
         ...
         * "nonceStr" : "adssdasssd13d", //随机串 "package" :
        ...
        public string CreateBizPackage()
        {
            ...
            nativeObj.Add("appId", AppId);
            nativeObj.Add("package", GetCftPackage());
            nativeObj.Add("timeStamp", Convert.ToInt64(ts.TotalSeconds).ToString());//((DateTime.Now.ToUniversalTime().Ticks - 621355968000000000) / 10000000).ToString());
            nativeObj.Add("nonceStr", Wxpay.CommonUtil.CreateNoncestr());
        ...
        public string CreateNativeUrl(string productid)
        {
            ...
            try
            {
                ...
                bizString = Wxpay.CommonUtil.FormatBizQueryParaMap(nativeObj, false);
        ...
        public string CreateNativePackage(string retcode, string reterrmsg)
        {
            ...
            return Wxpay.CommonUtil.ArrayToXml(nativeObj);
        ...
        /// </summary>
        /// <returns></returns>
        public string CreateDelivery(string openId, string transid, string out_trade_no)
        {
            Dictionary<string, string> nativeObj = new Dictionary<string, string>();
            nativeObj.Add("appid", AppId);
            ...
            nativeObj.Add("app_signature", sign);
            nativeObj.Add("sign_method", SignType);
            var entries = nativeObj.Select(d => string.Format("\"{0}\": \"{1}\"", d.Key, d.Value));
            return "{" + string.Join(",", entries.ToArray()) + "}";
        }
