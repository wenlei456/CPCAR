...
function InitCarSelect() {
    ...
    if (b) {
        if (b !== "-1") {
            ...
            $("#nav_brands").val(b);
            var carBrand = $("#nav_brands").val();
            
            getCarBrandOpen(carBrand);

            // proBrandChange();
            //alert($("#nav_proModelNew"));

           // $("#nav_proModelNew").val(m);
        ...
        $("#nav_brands ").val($("#nav_brands option:first").val());
        $("#nav_proModel").val("-1");
        $("#nav_proYear").val("-1");
        //$("#nav_brands").val(b);
        //var carBrand = $("#nav_brands").val();
        ...
        //$("#nav_proModelNew").val(m);
        //var carBrand = $("#nav_proModelNew").val();
        //getCarBrandYear(carBrand)
        //proBrandChange();
         //proModelChange();
    }
...
function selectCarTypeNew(id) {
    var tid = id;
    var carBrand = $("#nav_brands").val();
    var carModel = $("#nav_proModelNew").val();
    var carYear = $("#nav_proYear").val();
    window.location.href = "/Product/TypeListByCar?t=" + tid + "&&b=" + carBrand + "&&m=" + carModel + "&&y=" + carYear;
}
...
function getCarBrandOpen(brand) {
    if (brand !== "-1") {

        var options = "";
        $.post("/Common/GetPartModel/", { brand: brand }, function (data) {
            $("#nav_proModelNew option").remove();
            $("#nav_proModelNew").append("<option value='-1'>选择汽车型号</option>");
            for (var i = 0; i < data.length; i++) {
                options = "<option value='" + data[i].model + "'>" + data[i].model + "</option>";
                $("#nav_proModelNew").append(options);//Model
            }
...
function getCarBrandYearOpen(model) {
    if (brand !== "-1") {
        var brand = $("#nav_brands").val();
        var options = "";
        $.post("/Common/GetPartModel2/", { brand: brand, model: model }, function (data) {
            $("#nav_proYear option").remove();
            $("#nav_proYear").append("<option value='-1'>选择出产年份</option>");
            for (var i = 0; i < data.length; i++) {
...
function getCarBrandYear(model) {
    if (brand !== "-1") {
    var brand = $("#nav_brands").val();
    var options = "";
    $.post("/Common/GetPartModel2/", { brand: brand, model: model }, function (data) {
